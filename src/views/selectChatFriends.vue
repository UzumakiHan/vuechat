<template>
	<div class="mail-list">
		<van-nav-bar title="选择好友" :fixed="true" style="z-index:99" left-text="返回" left-arrow @click-left="onClickLeft">

		</van-nav-bar>
		<van-index-bar highlight-color="#fb6463" style="margin-top:50px;z-index:97;margin-bottom:70px" v-if="mailList.length > 0">
			<div v-for="(mailName,index) in LastMailList" :key="index">
				...
				<van-checkbox-group v-model="selectResult">
					<van-cell-group>
						<van-cell v-for="(item,cindex) in mailName" :key="cindex" clickable @click="toggle(item)">
			...
			<van-cell class="btn-cell">
				...
				<van-button type="info" @click="toggleAll" style="margin:0 10px;">反选</van-button>
				<van-button type="info" @click="sureAddChat" color="#337ab7">确定</van-button>
			</van-cell>
