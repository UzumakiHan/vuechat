		...
		<div class="logincontent">
			...
			<van-cell-group>
				...
				<van-row>
					...
					<van-col span="6">
						<div ref="svgRef"  class="svg-box" @click="getCaptcha"></div>
					</van-col>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getCaptcha() {
				const res = await getCaptchaSvg();
				this.$refs.svgRef.innerHTML=res.data.captchaSvg
				this.svgcaptchaText = res.data.captchaText
				...
				if (this.isHaveAccount) {
					if (
						...
						const userInfo = {
							...
							svgcaptchaText:this.svgcaptchaText
						};
